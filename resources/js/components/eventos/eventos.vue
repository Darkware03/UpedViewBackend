<template>
    <div class="container">
        <v-dialog persistent v-model="modal2" width="500">
            <v-card class="align-center">
                <v-card-title>Asistencias a eventos</v-card-title>
                <v-card>
                    <v-data-table
                ...
                <v-card-actions class="pb-4">
                    <v-spacer></v-spacer>
                    <v-btn color="red darken-1" text @click="modal2=false">Close</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        <v-dialog
            ...
            <v-card class="align-center">
                <v-card-title class="">Crear Noticia</v-card-title>
                <v-card >
                    <v-form
                        ...
                        ></v-text-field>
                        <label>Fecha</label>
                        <input type="date" v-model="fecha_evento"/><br><br>
                        <v-text-field
                                label="Lugar"
                                v-model="lugar"
                                required
        ...
        methods: {
            enviarDatos() {
              ...
              Iterar sobre cualquier archivo enviado anexando los archivos a los datos del formulario.
                ...
                formData.append('fecha_evento',this.fecha_evento)
                if (this.id !==null){
                ...
                axios.post("api/Eventos/posteventos",formData, {
                    headers: {
            ...
            getFiles(){
                let me=this;
                axios.post('api/Eventos/geteventos')
                    .then(function (response) {
                        me.noticias=response.data.data
                        me.asistencias=response.data.data2
            ...
            eliminar(item){
                this.$swal.fire({
                    ...
                    if (result.value) {
                        axios.post('api/Eventos/deleteEventos',{id:item.id})
                            .then(response => {
                                this.limpiar()
                                this.getFiles()
                                this.$swal.fire(
